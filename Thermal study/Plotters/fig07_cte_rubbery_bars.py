...
==========================
Rubbery-plateau CTE bar chart.
  PEEK: 160–200 °C window,  PPS: 105–140 °C window.
...
# ── Configuration ──────────────────────────────────────────────────────────────
FIGURE_BASENAME = "fig07_cte_rubbery_bars"
FIG_WIDTH_MM    = 178
ROW_HEIGHT_MM   = 55
...
DVs = [("alpha_rub_zz", r"$\alpha_{zz}^{\rm rubbery}$ [ppm °C$^{-1}$]"),
       ("alpha_rub_yy", r"$\alpha_{yy}^{\rm rubbery}$ [ppm °C$^{-1}$]")]
...
for i_c, mat in enumerate(materials):
    ...
    for i_r, (dv, ylabel) in enumerate(DVs):
        ...
        # Check if any rubbery data exists for this material
