...
===================================
Apparent through-thickness thermal conductivity k [W m⁻¹ K⁻¹] in the
SPECIMEN coordinate system.

Same layout as fig10 (columns = materials, x-axis = orientation), but bars
are coloured by the SC direction that heat flows through in each orientation:
    O1  →  Transraster  (green)
    O2  →  Raster       (red)
    O3  →  Build        (blue)

A reference dashed line marks k_ref (configurable below).
High χ = full colour, Low χ = muted.  Dots = individual replicates.
Numeric k values are annotated above each bar.
...
# ── Configuration ──────────────────────────────────────────────────────────────
FIGURE_BASENAME = "figSC10_thermal_conductivity_k"
FIG_WIDTH_MM    = 178
FIG_HEIGHT_MM   = 70
K_REF           = 0.286          # reference k [W m⁻¹ K⁻¹]; set None to omit
...
y_label = r"Apparent through-thickness $k$ [W m$^{-1}$ K$^{-1}$]"
...
for i_c, mat in enumerate(materials):
    ...
    for i_o, ori in enumerate(oris):
        ...
        crysts_present = [c for c in CRYST_ORDER if c in sub["crystallinity"].values]
        for i_cryst, cryst in enumerate(crysts_present):
            ...
            ax.bar(x, mean_k, width=bar_width * 0.9,
                   yerr=ci, color=dir_col, alpha=alpha_bar,
            ...
            # numeric k annotation above bar (above error bar cap)
            ax.text(x, mean_k + ci + 0.005, f"{mean_k:.3f}",
                    ha="center", va="bottom", fontsize=6.5,
                    color="k", zorder=5)

    # k_ref reference line
    if K_REF is not None:
        x_lo = x_centres[0] - group_gap * 0.5
        x_hi = x_centres[-1] + group_gap * 0.5
        ax.hlines(K_REF, x_lo, x_hi,
                  color="#c0392b", ls="--", lw=0.9, zorder=1)

    # X-tick labels: "O1\n(Transraster)" etc.
    tick_labels = [
        f"O{ori}\n({sc_direction_label(heated_direction(ori))})"
        for ori in oris
    ]
