...
========================
Per-extensometer εzz strain vs normalised time.
E0 = front (hot side), E1 = mid, E2 = back (cool side).
...
# ── Configuration ──────────────────────────────────────────────────────────────
FIGURE_BASENAME = "fig03_ext_strain_zz"
FIG_WIDTH_MM    = 178
ROW_HEIGHT_MM   = 48
N_TNORM         = 300
...
means = {k: np.nanmean(np.vstack(v), axis=0) for k, v in buckets.items()}
cis   = {}
        ...
        for s in VERT_SENSORS:
            ...
            for cryst in CRYST_ORDER:
                ...
                ax.plot(t_grid[valid], mu[valid],
                        ...
                        label=(EXT_LABELS_ZZ[s] if cryst == "High" else None))
                ax.fill_between(
                    t_grid[valid],
