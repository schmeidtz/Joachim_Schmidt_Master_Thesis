...
============================
Rubbery-regime CTE bar chart in the SPECIMEN coordinate system.

Same remapping as figSC06 but uses alpha_rub_zz / alpha_rub_yy.
...
# ── Configuration ──────────────────────────────────────────────────────────────
FIGURE_BASENAME = "figSC07_cte_rubbery_bars"
FIG_WIDTH_MM    = 178
ROW_HEIGHT_MM   = 65
...
for _, r in metrics_df.iterrows():
    ...
    pairs = [
        (zz_direction(ori), r.get("alpha_rub_zz", np.nan)),
        (yy_direction(ori), r.get("alpha_rub_yy", np.nan)),
    ]
...
if sc_df.empty:
    print("No rubbery CTE data — all alpha_rub values are NaN. "
          "Rubbery plateau may not have been reached for these specimens.")
