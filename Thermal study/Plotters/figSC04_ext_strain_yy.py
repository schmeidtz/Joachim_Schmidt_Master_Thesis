...
=========================
Per-extensometer εyy strain vs normalised time in the SPECIMEN coordinate system.

HORIZ sensors (E3–E5) measure the DIC yy direction, which maps to physical
specimen directions depending on orientation:
    O1 : yy → transraster
    O2 : yy → raster
    O3 : yy → build

Each orientation contributes to a different physical direction, giving three
separate columns.
...
Sensor colour = position (E3 top, E4 mid, E5 bottom/plate).
...
# ── Configuration ──────────────────────────────────────────────────────────────
FIGURE_BASENAME = "figSC04_ext_strain_yy"
FIG_WIDTH_MM    = 178
ROW_HEIGHT_MM   = 48
N_TNORM         = 300
...
for name, res in processed_results.items():
    ...
    sc_dir = yy_direction(ori)          # physical direction for HORIZ sensors
